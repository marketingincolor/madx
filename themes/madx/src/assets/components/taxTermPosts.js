									...
									<div class="grid-x grid-margin-x grid-margin-y">
									  <div class="small-12 cell text-center" v-if="loading">
										...
										<div class="medium-6 large-4 cell module auto-height animated fadeIn" v-for="post in taxPosts">
											<a @click="scrollToProducts(true);getSinglePost(post.id)"><div class="module-bg product-div-image" v-bind:style="{backgroundImage: 'url(' + post._embedded['wp:featuredmedia'][0].source_url + ')'}"></a>
											<div class="meta">
												<a @click="scrollToProducts(true);getSinglePost(post.id)"><h4 class="blue product-h4-image" v-html="post.title.rendered"></h4></a>
												<div class="content" v-html="$options.filters.limitWords(post.content.rendered,25)"></div>
												<a @click="scrollToProducts(true);getSinglePost(post.id)" class="read-more product-read-more">View Product Details &nbsp;<i class="far fa-long-arrow-right"></i></a>
											</div>
										</div>
									</div>
								</div>
